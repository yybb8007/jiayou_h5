<template>
	<div>
		...
		<section id="wrapper" class="sy_main" @click="callback">
			<div id="ms_banner" class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide" v-for="item in data.lunbo"><img :src="item.lunbo_imgurl" alt="" :ID="item.data" :target="item.target" /></div>
				</div>
			...
			<main id="ms_main">
				...
				<ul>
						...
						<div class="data_message">
							<ul>
								<li v-for="item in data[0].goods_detaill_arr" :ID="item.goods_id" :target="item.target">
									<img v-lazy="item.goods_thumb" alt="" :ID="item.goods_id" :target="3" />
									<p class="message_name" :ID="item.goods_id" :target="3">{{ item.goods_name }}</p>
					...
					<li class="data_three data_noe">
						<div class="data_message">
							<ul>
								<li v-for="item in data[2].goods_detaill_arr" :ID="item.goods_id" :target="item.target">
									<img v-lazy="item.goods_thumb" alt="" :ID="item.goods_id" :target="3" />
									<p class="message_name" :ID="item.goods_id" :target="3">{{ item.goods_name }}</p>
									<p class="message_price">￥<span class="newPrice" :ID="item.goods_id" :target="item.target">{{ item.shop_price }}</span><span class="oldPrice">￥{{ item.market_price }}</span></p>
								</li>
